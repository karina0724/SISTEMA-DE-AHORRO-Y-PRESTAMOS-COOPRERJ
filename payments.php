		...
		<div class="card">
			<div class="card-header">
				<large class="card-title">
					<b>Lista de pagos</b>
					<button class="btn btn-success btn-sm btn-block col-md-2 float-right" type="button" id="new_payments"><i class="fa fa-plus"></i> Nuevo pago</button>
				</large>
				...
				<table class="table table-bordered" id="loan-list">
					...
					<thead>
						<tr>
							...
							<th class="text-center">Numero de referencia del prestamo</th>
...
<script>
	...
	$('#new_payments').click(function(){
		uni_modal("Pago nuevo","manage_payment.php",'mid-large')
	})
	$('.edit_payment').click(function(){
		uni_modal("Editar pago","manage_payment.php?id="+$(this).attr('data-id'),'mid-large')
	})
	$('.delete_payment').click(function(){
		_conf("¿Estás seguro de eliminar estos datos?","delete_payment",[$(this).attr('data-id')])
	})
function delete_payment($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Pago eliminado correctamente",'success')
					setTimeout(function(){
						location.reload()
					},1500)
