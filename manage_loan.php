...
<div class="container-fluid">
	...
	<form action="" id="loan-application">
		...
		<div class="row">
			<div class="col-md-6">
				<label class="control-label">Plan de préstamo</label>
				<?php
				...
				<small>meses[ interesar%,multa% ]</small>
			</div>
		<div class="form-group col-md-6">
			<label class="control-label">Monto del préstamo</label>
		...
		<div class="row">
			<div class="form-group col-md-6">
				...
				<select class="custom-select browser-default" name="status">
					<option value="0" <?php echo $status == 0 ? "selected" : '' ?>>Para su aprobación</option>
					<option value="1" <?php echo $status == 1 ? "selected" : '' ?>>Aprobado</option>
					<?php if($status !='4' ): ?>
					<option value="2" <?php echo $status == 2 ? "selected" : '' ?>>Liberado</option>
					<?php endif ?>
					<?php if($status =='2' ): ?>
					<option value="3" <?php echo $status == 3 ? "selected" : '' ?>>Completado</option>
					<?php endif ?>
					<?php if($status !='2' ): ?>
					<option value="4" <?php echo $status == 4 ? "selected" : '' ?>>Denegado</option>
					<?php endif ?>
				</select>
			</div>
		</div>
	...
	$('#loan-application').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp ==1 ){
					$('.modal').modal('hide')
					alert_toast("Los datos del préstamo se guardaron correctamente.","success")
					setTimeout(function(){
						location.reload();
					},1500)
				}
