				...
				<table class="table table-bordered w-100" id="saving-list">
					...
					<tbody>
						<?php
							...
							$qry = $conn->query("SELECT s.id, concat(b.firstname,' ',b.lastname) as 'borrower_name', b.address, b.contact_no, s.borrower_id, s.saving_type_id as saving_type, st.name, s.current_balance FROM saving s INNER JOIN saving_types st ON s.saving_type_id = st.id INNER JOIN borrowers b ON s.borrower_id = b.id;");
