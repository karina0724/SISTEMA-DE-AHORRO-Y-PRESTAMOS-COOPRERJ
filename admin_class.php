...
Class Action {
	...
	function get_users(){
		...
		$tmp = [];
		if($query->num_rows > 0) {
			while($row = $query->fetch_assoc()) $tmp[] = $row;
		}
		return $tmp;
	}
	...
	function save_saving(){
		extract($_POST);
			$data = " saving_type_id = $saving_type_id ";
			$data .= " , borrower_id = '$borrower_id' ";
			$data .= " , amount = '$amount' ";
			$data .= " , current_balance  = '$current_balance' ";
		if(empty($id)){
			$save = $this->db->query("INSERT INTO saving set ".$data);
		}else{
			$save = $this->db->query("UPDATE saving set ".$data." where id = ".$id);
	...
	function delete_saving(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM saving where id = ".$id);
		if($delete)
			return 1;
	}
}